...
module.exports = function(execute) {
	...
	return {
		 ...
		 * @param callback Callback(err, json)
		 */
		findByTwitter: function(twitterHandle, options, callback) {
            if(_.isFunction(options)){
                callback    = options;
                options     = {};
            }

            var requestParams   = _.defaults({twitter: twitterHandle},options);
            executeRequest(requestParams, callback);
		},
		 ...
		 * @param callback Callback(err, json)
		 */
		findByFacebook: function(username, options, callback) {
            if(_.isFunction(options)){
                callback    = options;
                options     = {};
            }

            var requestParams   = _.defaults({facebookUsername: username},options);
			executeRequest(requestParams, callback);
		},
